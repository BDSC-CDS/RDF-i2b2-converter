...
def transfer_obs_numerical_values(
    output_tables_loc, migration_filename, observation_filename
):
    def transfer(row):
        vv = row["INSTANCE_NUM"]
        if vv % 2000 == 0:
        ...
        curgrp = gps.get_group(
            tuple(row.loc[["PATIENT_NUM", "CONCEPT_CD", "INSTANCE_NUM"]])
        )
    ...
    def get_dests(row, curgrp):
        dest = curgrp.loc[
            curgrp["MODIFIER_CD"].isin(destinations)
            | (
    ...
    def get_shot(dests, row):
        ...
        if dests_idx.size > 1:
            raise Exception(
                "Cannot relocate", row["C_FULLNAME"], "to more than one location"
            )
            ...
            if len(destinations) == 1:
                ...
                print(
                    "Couldn't relocate",
                    row["MODIFIER_CD"],
                    "because no available destination in the same instance (patient, encounter:",
                    row["PATIENT_NUM"],
                    row["ENCOUNTER_NUM"],
                    ")",
                )
            ...
            tmp = pd.concat(
                [
                    dests[COLUMNS_TMP].squeeze(),
                    pd.Series({"INDICES_TO_RM": dests_idx.values[0]}),
                ]
            )
        # tmp.rename(row.name)
        return tmp
    ...
    try:
        with open(output_tables_loc + migration_filename) as json_file:
            migrations = json.load(json_file)
    except:
        print("No migration logs found, skipping")
        return
    df = pd.read_csv(output_tables_loc + observation_filename)
    ...
    for el, destinations in migrations.items():
        ...
        print(
            "migrating value fields for",
            el,
            ",",
            tbmigrated_rows.shape,
            "lines affected",
        )
    ...
    df.to_csv(output_tables_loc + observation_filename, index=False)
